		...
		const phases = {
			...
			testRunner: async () => {
				...
				const res = await tester({
					git: versioning,
					app: targetApp,
					reportDir: `${this.opts.logsDir}/test-reports/${targetName}`,
				});

				if (res.pass) {
					logger.log('[%s] All tests passing on latest commit for application %s, pulling', localeDateString(), targetName);
