...
class Worker {
	...
	processRequest(req) {
		...
		if (!Worker.oldSpawns[targetName]) Worker.oldSpawns[targetName] = {};
		...
		const phases = {
			...
			preHook: (cb) => {
				...
				const oldChild = Worker.oldSpawns[targetName].prehook;
				...
				const child = spawnAsExec(targetApp.prehook, execOptions,
					logCallback(() => {
						Worker.oldSpawns[targetName].prehook = undefined;
						cb();
					}, '[%s] Prehook command has been successfuly executed for app %s', localeDateString(), targetName),
				);

				Worker.oldSpawns[targetName].prehook = child;
			},
